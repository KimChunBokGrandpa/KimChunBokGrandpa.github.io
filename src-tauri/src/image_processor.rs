...
// ─── Constants & LUT logic ───
const BITS: u32 = 5;
const _LEVELS: u32 = 1 << BITS; // 32
const SHIFT: u32 = 8 - BITS;    // 3
...
fn color_distance(cr: i32, cg: i32, cb: i32, r: i32, g: i32, b: i32) -> i32 {
    ...
    let dg = cg - g;
    let db = cb - b;
    // Weighted Euclidean distance (R:2, G:4, B:3)
            ...
            for bi in 0..32 {
                ...
                for (pi, color) in palette.iter().enumerate() {
                    let d = color_distance(
                        ...
                        r_val as i32, g_val as i32, b_val as i32
                    );
                    if d == 0 {
                        nearest_idx = pi;
                        break;
...
#[tauri::command]
pub fn process_image_rs(data: Vec<u8>, req: ProcessRequest) -> Result<Vec<u8>, String> {
    let width = req.width as usize;
    let height = req.height as usize;
    let effective_pixel_size = std::cmp::max(1, req.pixel_size as usize);
    
    // Original path: early return or no palette
    ...
    if req.dither_type == "floyd_steinberg" && use_palette {
        return Ok(apply_floyd_steinberg(&data, width, height, effective_pixel_size, &req.palette));
    }
    
    if req.dither_type == "ordered" && use_palette {
        return Ok(apply_ordered_dither(&data, width, height, effective_pixel_size, &req.palette));
    }
    
    // Default no-dither path
    Ok(apply_default(&data, width, height, effective_pixel_size, if use_palette { Some(&req.palette) } else { None }))
}
...
fn apply_ordered_dither(
    ...
    for y in (0..height).step_by(pixel_size) {
        for x in (0..width).step_by(pixel_size) {
            ...
            let bx = x / pixel_size;
            let by = y / pixel_size;
            let thresh = (BAYER_8X8[((by & 7) * 8 + (bx & 7)) as usize] as f32 / 64.0 - 0.5) * spread as f32;
...
fn apply_floyd_steinberg(
    ...
    let rw = (width + pixel_size - 1) / pixel_size;
    let rh = (height + pixel_size - 1) / pixel_size;
            ...
            for by in 0..block_h {
                ...
                for bx in 0..block_w {
                    ...
                    if pixels[alpha_idx] < 128 {
                        continue; // transparent out_data initialized to 0
                    }
