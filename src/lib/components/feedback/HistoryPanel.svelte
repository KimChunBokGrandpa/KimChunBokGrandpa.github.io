<script lang="ts">
  /**
   * HistoryPanel — Displays the undo/redo timeline and allows jumping
   * to specific states.
   */
  import { getPaletteName } from '$lib/utils/palettes';
  import { i18n } from '$lib/i18n/index.svelte';
  import { buildShortcutLabel } from '$lib/utils/platformShortcuts';
  import { tooltip } from '$lib/utils/tooltip';
  import type { ProcessingSettings } from '$lib/types';

  let {
    history,
    redoHistory,
    currentSettings,
    onJumpToHistory,
    onUndo,
    onRedo,
  }: {
    history: ProcessingSettings[];
    redoHistory: ProcessingSettings[];
    currentSettings: ProcessingSettings;
    onJumpToHistory: (index: number, isRedo: boolean) => void;
    onUndo: () => void;
    onRedo: () => void;
  } = $props();

  let listEl = $state<HTMLDivElement | null>(null);

  // Auto-scroll to the current item when history changes
  $effect(() => {
    // Track history length to trigger on changes
    const _len = history.length + redoHistory.length;
    if (listEl) {
      // Use tick to ensure DOM is updated
      requestAnimationFrame(() => {
        const currentEl = listEl?.querySelector('.current');
        currentEl?.scrollIntoView({ block: 'nearest' });
      });
    }
  });

  // Helper to concisely describe a setting state
  function describeSettings(s: ProcessingSettings) {
    let desc = i18n.t('history_pixel').replace('{0}', String(s.pixelSize)) + ', ';
    desc += getPaletteName(s.palette);
    if (s.crtEffect !== 'none') desc += ' ' + i18n.t('history_crt');
    if (s.glitchFilters.length > 0) desc += ' ' + i18n.t('history_glitch').replace('{0}', String(s.glitchFilters.length));
    return desc;
  }

  let undoShortcutLabel = $derived(buildShortcutLabel(['Primary', 'Z']));
  let redoShortcutLabel = $derived(buildShortcutLabel(['Primary', 'Shift', 'Z']));
</script>

<div class="history-panel">
  <div class="history-controls w98-toolbar">
    <button class="w98-inline-button w98-button--thin" onclick={onUndo} disabled={history.length === 0} title={`${i18n.t('undo')} (${undoShortcutLabel})`} use:tooltip>
      <span class="w98-structural-glyph" aria-hidden="true">↩</span>
      <span>{i18n.t('undo')}</span>
    </button>
    <button class="w98-inline-button w98-button--thin" onclick={onRedo} disabled={redoHistory.length === 0} title={`${i18n.t('redo')} (${redoShortcutLabel})`} use:tooltip>
      <span class="w98-structural-glyph" aria-hidden="true">↪</span>
      <span>{i18n.t('redo')}</span>
    </button>
  </div>

  <div class="history-list" bind:this={listEl}>
    {#if history.length === 0 && redoHistory.length === 0}
      <div class="history-empty w98-note">
        <span class="history-empty-icon w98-emoji">📜</span>
        <p class="history-empty-text">{i18n.t('history_empty')}</p>
        <p class="history-empty-hint">{i18n.t('history_empty_hint')}</p>
      </div>
    {:else}
      <!-- Past states -->
      {#each history as step, i}
        <button type="button" class="history-item past" onclick={() => onJumpToHistory(i, false)}>
          <span class="step-num">{i + 1}</span>
          <span class="step-desc">{describeSettings(step)}</span>
        </button>
      {/each}
    {/if}

    <!-- Current state -->
    <div class="history-item current">
      <span class="step-num">{history.length + 1}</span>
      <span class="step-desc">{describeSettings(currentSettings)}</span>
      <span class="current-badge">{i18n.t('current')}</span>
    </div>

    <!-- Future (redo) states -->
    <!-- redoHistory acts like a stack, so the top of the stack (last element) is the NEXT step -->
    {#each [...redoHistory].reverse() as step, i}
      <button type="button" class="history-item future" onclick={() => onJumpToHistory(redoHistory.length - 1 - i, true)}>
        <span class="step-num">{history.length + 2 + i}</span>
        <span class="step-desc">{describeSettings(step)}</span>
      </button>
    {/each}
  </div>
</div>

<style>
  .history-panel {
    display: flex;
    flex-direction: column;
    gap: var(--w98-space-4);
    height: 100%;
    padding: var(--w98-space-4);
    background: var(--w98-surface);
    font-size: var(--w98-font-size-base);
    overflow: hidden;
  }

  .history-controls {
    gap: var(--w98-space-4);
  }

  .history-controls button {
    flex: 1;
  }

  .history-list {
    flex: 1;
    background: var(--w98-surface-white);
    box-shadow: var(--w98-inset);
    overflow-y: auto;
    display: flex;
    flex-direction: column;
    padding: var(--w98-space-4);
  }

  .history-item {
    display: flex;
    align-items: center;
    gap: var(--w98-space-6);
    padding: var(--w98-space-4) var(--w98-space-6);
    margin-bottom: var(--w98-space-2);
    text-align: left;
    background: var(--w98-surface-white);
    border: none;
    box-shadow: var(--w98-outset-thin);
    cursor: pointer;
    font-family: inherit;
    font-size: var(--w98-font-size-base);
    width: 100%;
  }
  .history-item:last-child {
    margin-bottom: 0;
  }

  .history-item:hover:not(.current) {
    background: var(--w98-highlight);
    color: #fff;
  }

  .history-item.past {
    color: var(--w98-text-secondary);
  }

  .history-item.current {
    background: var(--w98-surface-active);
    box-shadow: var(--w98-inset-thin);
    font-weight: bold;
    cursor: default;
  }

  .history-item.future {
    color: var(--w98-text-hint);
  }

  .history-item:hover.future {
    color: #fff; /* override italics color on hover */
  }

  .step-num {
    display: inline-flex;
    align-items: center;
    justify-content: center;
    width: 20px;
    min-width: 20px;
    min-height: 16px;
    background: var(--w98-surface);
    box-shadow: var(--w98-inset-thin);
    color: var(--w98-shadow-808);
    text-align: center;
  }
  .history-item:hover:not(.current) .step-num {
    color: var(--w98-surface);
  }
  .history-item.current .step-num {
    background: var(--w98-surface-white);
  }

  .step-desc {
    flex: 1;
    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
  }

  .current-badge {
    font-size: var(--w98-font-size-caption);
    padding: 1px var(--w98-space-4);
    background: var(--w98-highlight);
    color: #fff;
    box-shadow: var(--w98-inset-thin);
    letter-spacing: 0.4px;
    text-transform: uppercase;
  }

  .history-empty {
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
    padding: var(--w98-space-16) var(--w98-space-8);
    gap: var(--w98-space-4);
    text-align: center;
  }
  .history-empty-icon {
    font-size: 24px;
    color: var(--w98-text-hint);
  }
  .history-empty-text {
    font-size: var(--w98-font-size-base);
    color: var(--w98-text-muted);
    margin: 0;
  }
  .history-empty-hint {
    font-size: var(--w98-font-size-sm);
    color: var(--w98-text-disabled);
    margin: 0;
  }
</style>
